...
class Setup(States):
    ...
    def enter(self, args):
        ...
        self.csv_file, self.csv_writer = Util.setupCSV(self.user_ID)
        # Gets probability distribution from file
        self.probs = Util.extractProb()
        self.overall_total = 0
    
    def exit(self):
        # Passes all values into play state
        return [self.user_ID, self.socket, self.csv_file, self.csv_writer, self.probs, self.num_rounds, self.overall_total, self.active_response]
...
class Play(States):
    ...
    def enter(self, args):
        ...
        self.user_ID, self.socket, self.csv_file, self.csv_writer, self.probs, self.num_rounds_left, self.overall_total, self.active_response = args
    ...
    def exit(self):
        ...
        self.csv_writer.writerow({'num_keypresses' : self.balloon_size, 'round_money_won' : self.round_total * (not self.popped), 'total_money_won' : self.overall_total, 'popped' : self.popped})
        # Passes global values into next state (whilst decreasing number of rounds remaining)
        return [self.user_ID, self.socket, self.csv_file, self.csv_writer, self.probs, self.num_rounds_left - 1, self.overall_total, self.round_total, self.balloon_size, self.active_response]
    ...
    def getEvent(self, event):
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RIGHT:
                ...
                self.popped = random.randint(10 - self.probs[self.balloon_size] + 1, 10) == 10
    ...
    def draw(self, screen):
        ...
        Util.drawImage(screen, f"images/balloon{self.balloon_size + 1}high.png", screen.get_width() // 2, screen.get_height()  // 2, (400, 540))
...
class EndRound(States):
    ...
    def enter(self, args):
        ...
        self.user_ID, self.socket, self.csv_file, self.csv_writer, self.probs, self.num_rounds_left, self.overall_total, self.round_total, self.balloon_size, self.active_response = args
    ...
    def exit(self):
        ...
        return [self.user_ID, self.socket, self.csv_file, self.csv_writer, self.probs, self.num_rounds_left, self.overall_total, self.active_response]
    ...
    def cleanup(self):
        ...
        del self.user_ID, self.socket, self.csv_file, self.csv_writer, self.probs, self.num_rounds_left, self.overall_total, self.round_total, self.balloon_size, self.active_response
...
class Cashed(EndRound):
    ...
    def draw(self, screen):
        ...
        # Only runs if active response is on
        if self.active_response:
...
class Control():
    ...
    def __init__(self, **settings):
        ...
        # Sets window caption
        pygame.display.set_caption("Balloon Analogue Risk Task (single-risk)")
